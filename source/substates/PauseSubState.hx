...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		grid.alpha = 0;
		grid.cameras = [FlxG.cameras.list[FlxG.cameras.list.length-1]];
		...
		fnfBar.setGraphicSize(FlxG.width+100);
		fnfBar.updateHitbox();
		fnfBar.screenCenter();
		add(fnfBar);
		fnfBar.y -= 30;
		...
		tetoa = new FlxSprite(1000 + 300, 0).loadGraphic(Paths.image('freeplay/tetoa', 'vocaloid'));
		tetoa.antialiasing = ClientPrefs.data.antialiasing;
		tetoa.screenCenter(Y);
		add(tetoa);

		songImage = new FlxSprite(850, 0).loadGraphic(Paths.image('week/' + states.ReadySongState.bgPath));
		...
		var levelInfo:FlxText = new FlxText(20, 15, 0, OeTools.getJsonData('FreeplaySongName', PlayState.SONG.song) + ' - ' + OeTools.getJsonData('FreeplayComposer', PlayState.SONG.song), 32);
		...
		var blueballedTxt:FlxText = new FlxText(20, 15 + 64, 0, OeTools.getJsonData('Main', 'pause_blueballed') + PlayState.deathCounter, 32);
		blueballedTxt.scrollFactor.set();
		blueballedTxt.setFormat(Paths.font('vcr.ttf'), 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 101, 0, OeTools.getJsonData('Main', 'pause_practice_mode').toUpperCase(), 32);
		...
		var chartingText:FlxText = new FlxText(20, 15 + 101, 0, OeTools.getJsonData('Main', 'pause_charting_mode').toUpperCase(), 32);
		...
		FlxTween.tween(tetoa, {x: 1000}, 0.6, {ease: FlxEase.quartInOut});
		FlxTween.tween(fnfBar, {x: 0}, 0.5, {ease: FlxEase.quartInOut});
		FlxTween.tween(songImage, {alpha: 1}, 0.6, {ease: FlxEase.quartInOut});
	...
	override function update(elapsed:Float)
	{
		...
		if (tetoa != null) tetoa.angle += elapsed * 20;
