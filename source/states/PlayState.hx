...
class PlayState extends MusicBeatState
{
	...
	public var vocaloidHealthBar:FlxSprite;

	public var scoreTxtList:Array<String> = [
		OeTools.getJsonData('Main', 'scoretext_score'),
		OeTools.getJsonData('Main', 'scoretext_misses'),
		OeTools.getJsonData('Main', 'scoretext_combo'),
		OeTools.getJsonData('Main', 'scoretext_max_combo'),
		OeTools.getJsonData('Main', 'scoretext_rating'),
		OeTools.getJsonData('Main', 'ratingfctext_sfc'),
		OeTools.getJsonData('Main', 'ratingfctext_gfc'),
		OeTools.getJsonData('Main', 'ratingfctext_fc'),
		OeTools.getJsonData('Main', 'ratingfctext_sdcb'),
		OeTools.getJsonData('Main', 'ratingfctext_clear'),
		OeTools.getJsonData('Main', 'ratingtext_perfect'),
		OeTools.getJsonData('Main', 'ratingtext_sick'),
		OeTools.getJsonData('Main', 'ratingtext_great'),
		OeTools.getJsonData('Main', 'ratingtext_nice'),
		OeTools.getJsonData('Main', 'ratingtext_good'),
		OeTools.getJsonData('Main', 'ratingtext_meh'),
		OeTools.getJsonData('Main', 'ratingtext_bruh'),
		OeTools.getJsonData('Main', 'ratingtext_bad'),
		OeTools.getJsonData('Main', 'ratingtext_shit'),
		OeTools.getJsonData('Main', 'ratingtext_you_suck')
	];
	public var maxcombo:Int = 0;
	var clickCount:Int = 0;
	var currentNFS:Int = 0;
	var maxNFS:Int = 0;
	...
	override public function create()
	{
		...
		vocaloidHealthBar = new FlxSprite(0, (ClientPrefs.data.downScroll ? -215 : 345)).loadGraphic(Paths.image('ui/healthBar-vocaloid', 'vocaloid'));
		vocaloidHealthBar.screenCenter(X);
		vocaloidHealthBar.scale.x = 0.5;
		vocaloidHealthBar.scale.y = 0.4;
		uiGroup.add(vocaloidHealthBar);
	...
	public dynamic function updateScoreText()
	{
		...
		if (!cpuControlled)
		{
			scoreTxt.text = 'NFS: ' + currentNFS + ' [' + scoreTxtList[3] + ': ' + maxNFS + ']'
			+ ' | ' + scoreTxtList[0] + ': ' + songScore
			+ ' | ' + scoreTxtList[1] + ': ' + songMisses
			+ ' | ' + scoreTxtList[2] + ': ' + combo
			+ ' [' + scoreTxtList[3] + ': '+ maxcombo +']'
			+ ' | ' + scoreTxtList[4] + ': ' + ratingName
	...
	public dynamic function fullComboFunction()
	{
		...
		ratingFC = "";
		if (sicks > 0) ratingFC = scoreTxtList[5];
		if (goods > 0) ratingFC = scoreTxtList[6];
		if (bads > 0 || shits > 0) ratingFC = scoreTxtList[7];
		if (songMisses > 0 && songMisses < 10) ratingFC = scoreTxtList[8];
		else if (songMisses >= 10) ratingFC = scoreTxtList[9];
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (vocaloidHealthBar != null && healthBar != null)
		{
			vocaloidHealthBar.alpha = healthBar.alpha;
			vocaloidHealthBar.visible = healthBar.visible;
		}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (!note.isSustainNote) clickCount++;
	...
	public function RecalculateRating(badHit:Bool = false, scoreBop:Bool = true) {
		...
		if(ret != LuaUtils.Function_Stop)
		{
			...
			if(totalPlayed != 0) //Prevent divide by 0
			{
				...
				// Rating Name
				if (ratingPercent >= 0 && ratingPercent < 0.2)
				{
					ratingName = scoreTxtList[19];
				}

				if (ratingPercent >= 0.2 && ratingPercent < 0.3)
				{
					ratingName = scoreTxtList[18];
				}

				if (ratingPercent >= 0.3 && ratingPercent < 0.4)
				{
					ratingName = scoreTxtList[17];
				}

				if (ratingPercent >= 0.4 && ratingPercent < 0.5)
				{
					ratingName = scoreTxtList[16];
				}

				if (ratingPercent >= 0.5 && ratingPercent < 0.6)
				{
					ratingName = scoreTxtList[15];
				}

				if (ratingPercent >= 0.6 && ratingPercent < 0.7)
				{
					ratingName = scoreTxtList[14];
				}

				if (ratingPercent >= 0.7 && ratingPercent < 0.8)
				{
					ratingName = scoreTxtList[13];
				}

				if (ratingPercent >= 0.8 && ratingPercent < 0.9)
				{
					ratingName = scoreTxtList[12];
				}

				if (ratingPercent >= 0.9 && ratingPercent < 1)
				{
					ratingName = scoreTxtList[11];
				}

				if (ratingPercent >= 1)
				{
					ratingName = scoreTxtList[10];
				}
			}
			fullComboFunction();
		}
