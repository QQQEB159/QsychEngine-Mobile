...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if (!initialized && FlxG.sound.music == null)
			FlxG.sound.playMusic(Paths.returnSound('music/freakyMenu', 'vocaloid'), 0);

		loadJsonData();
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menu/BG', 'vocaloid'));
		...
		logoBl.frames = Paths.getSparrowAtlas('title/logoBumpin', 'vocaloid');
		...
		blackScreen = new FlxSprite().loadGraphic(Paths.image('menu/BG', 'vocaloid'));
		//blackScreen.scale.set(FlxG.width, FlxG.height);
		blackScreen.setGraphicSize(FlxG.width);
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('credits/Baka', 'vocaloid'));
		ngSpr.visible = false;
		ngSpr.setGraphicSize(200);
		ngSpr.updateHitbox();
	...
	var logoPosition:FlxPoint = FlxPoint.get(-150, -180);
	...
	{
		#if MODS_ALLOWED
		var firstArray:Array<String> = Mods.mergeAllTextsNamed('data/languages/' + ClientPrefs.data.languages + '/text/introText.txt');
		#else
		var fullText:String = Paths.getTextFromFile('data/languages/' + ClientPrefs.data.languages + '/text/introText.txt', true);
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		if (canCreateText){
			...
			{
				var money:FlxText = new FlxText(0, 0, 0, textArray[i].toUpperCase(), 40);
				money.setFormat(Paths.font('vcr.ttf'), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				money.antialiasing = ClientPrefs.data.antialiasing;
				add(money);
				money.screenCenter(X);
				money.y += (i * 60) + 250 + offset;
	...
	{
		if (canCreateText){
			var coolText:FlxText = new FlxText(0, 0, 0, text.toUpperCase(), 40);
			...
			coolText.y += (textList.length * 60) + 250 + offset;

			textList.push(coolText);
		}
	}
	...
	override function beatHit()
	{
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText([OeTools.getJsonData("Main", "titlestate_1")]);
				case 4:
					addMoreText(OeTools.getJsonData("Main", "titlestate_2"));
				case 5:
					deleteCoolText();
				case 6:
					createCoolText([OeTools.getJsonData("Main", "titlestate_3")]);
				case 8:
					addMoreText(OeTools.getJsonData("Main", "titlestate_4"));
				case 9:
					deleteCoolText();
				case 10:
					createCoolText([OeTools.getJsonData("Main", "titlestate_5")], -40);
				case 12:
					addMoreText(OeTools.getJsonData("Main", "titlestate_6"), -40);
					ngSpr.visible = true;
				case 13:
					deleteCoolText();
					ngSpr.visible = false;
			...
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					default: //Go back to normal ugly ass boring GF
						...
						FlxG.sound.playMusic(Paths.music('freakyMenu', 'vocaloid'), 0);
						FlxG.sound.music.fadeIn(4, 0, 0.7);
						return;
				}
