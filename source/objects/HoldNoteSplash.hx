...
class HoldNoteSplash extends FlxSprite
{
	...
	public function new(?x:Float = 0, ?y:Float = 0, noteData:Int = 0)
	{
		...
		var shaderValues = Note.initializeGlobalRGBShader(noteData % Note.colArray.length);
		rgbShader.copyValues(shaderValues);
		if (PlayState.isPixelStage) rgbShader.pixelAmount = 6;
	}
}

class PixelHoldSplashShaderRef 
{
	public var shader:PixelHoldSplashShader = new PixelHoldSplashShader();
...
class PixelHoldSplashShader extends FlxShader
{
	@:glFragmentHeader('
		#pragma header
